    ...
    @Operation(
            ...
            description = """
                    ...
                    **강제 업데이트도 에러가 아니라 200 + 플래그**로 내려간다. `data.forceUpdate` 가 true 면
                    ...
                    `data.termsVersions` 는 현행 동의 버전 7종(약관 5종 + 법정 개별 동의 2종)이다. **클라이언트가 약관 버전을 하드코딩하지 않게 하려는 것**으로,
                    가입 시 동의 버전 기록과 약관 개정 시 재동의 판정(`GET /api/v1/users/me` 의 저장 버전과 비교)에 쓴다.

                    `devTestMsg`·`minAppVersion` 은 설정값이 빈 문자열이면 null 로 내려간다(클라이언트의 폴백 처리가 동작하도록).
                    **`termsVersions` 는 해당하지 않는다** — 비울 수 없는 값이라 기동에서 막고, 항상 실제 버전이 내려간다.

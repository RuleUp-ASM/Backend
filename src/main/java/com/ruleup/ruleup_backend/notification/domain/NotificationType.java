 ...
 * 공통 8절의 <b>코드 ↔ 그룹 ↔ 딥링크 매핑 표</b>가 21종이고, 여기에 공지({@code ANNOUNCEMENT},
 * 공통 #8 로 타입 편입)를 더해 22종이다. 마이그레이션 파일명
 * ({@code V34__notification_registry_22_types})의 숫자가 그것이다. 늘어난 한 종이 CS 답변 고지다.
 *
 * <p>근거는 <b>앱 운영 정책 §5.5</b> 이고, 이 상수의 속성(분류 일반 · 토글 계정 · 중복 인터벌
 * 없음)이 그 절의 값 그대로다. 그 문서의 미결 사항이 「알림 정책 §4 표에 CS 답변 행 추가 필요」를
 * 남겨 두었고 <b>아직 추가되지 않았다</b> — 그래서 정책 §4 표에도, 그것을 참조하는 공통 8절
 * 표에도 이 타입의 행이 없다. <b>코드가 문서보다 앞서 있는 상태</b>이며 두 표가 갱신되면 맞는다.
 *

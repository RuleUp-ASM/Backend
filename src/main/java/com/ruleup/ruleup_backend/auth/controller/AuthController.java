package com.ruleup.ruleup_backend.auth.controller;

import com.ruleup.ruleup_backend.auth.service.AuthService;
import com.ruleup.ruleup_backend.auth.dto.LogoutRequest;
import com.ruleup.ruleup_backend.auth.dto.NicknameAvailabilityResponse;
import com.ruleup.ruleup_backend.auth.service.NicknameCheckRateLimiter;
import com.ruleup.ruleup_backend.auth.dto.NicknameCheckRequest;
import com.ruleup.ruleup_backend.auth.dto.OAuthLoginRequest;
import com.ruleup.ruleup_backend.auth.dto.OAuthLoginResponse;
import com.ruleup.ruleup_backend.auth.dto.RefreshRequest;
import com.ruleup.ruleup_backend.auth.dto.SignupRequest;
import com.ruleup.ruleup_backend.auth.dto.SignupResponse;
import com.ruleup.ruleup_backend.auth.dto.TokenResponse;
...
public class AuthController {
            ...
            description = """
                    ...
                    **`signupToken` 은 1회용**이다. 이미 쓴 토큰을 다시 제출하면 400 `INVALID_SIGNUP_TOKEN` 이고,
                    ...
                    - `birthDate` — 필수. `YYYY-MM-DD` 이고 미래일 수 없다. **만 14세 미만은 가입 불가**(법적 요구사항).
                      가입 후에는 수정할 수 없다.
                    - `gender` — 필수 필드. `MALE` / `FEMALE` 만 받는다.
                    - `agreements` — 5종 전부 보낸다. 필수 3종(이용약관·개인정보·위치기반)이 모두 `agreed=true` 여야 가입되고,
                      선택 2종(마케팅·이벤트)은 전부 거부해도 가입된다. `version` 을 생략하면 서버 현행 버전으로 기록한다.
                      야간 푸시 동의 약관은 2026-08-28 폐지됐다 — 야간은 알림 분류별로 일괄 처리한다.
    ...
    @Operation(
            ...
            description = """
                    ...
                    **클라이언트는 refresh 호출을 반드시 single-flight로 직렬화해야 한다.** 같은 refreshToken 을 두 요청이
                    동시에 제출하면 하나만 회전에 성공하고, 후발 요청은 재사용으로 감지되어 선발 요청이 받은 새 토큰까지
                    family 전체가 폐기된다. 새 refreshToken 저장도 응답 처리와 함께 원자적으로 덮어써야 한다.

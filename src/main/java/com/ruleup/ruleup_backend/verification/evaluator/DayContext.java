 ...
 *  - signals: 그 귀속일의 <b>원본 신호 전부</b>(델타가 아니다). 평가기가 자기 type만 골라 쓰며,
 *    매 평가마다 처음부터 다시 계산한다 — 요약 이월이 없어야 분할 전송 순서에 판정이 흔들리지 않는다.
 ...
 *  - userId/challengeId: 앱이 보내는 지오펜스 id({@code "{userId}#{challengeId}#{index}"})를 이 멤버의 것으로
 *    알아보는 데 쓴다. 없으면 memberId 형식만 인정한다.
 */
public record DayContext(
    ...
    /** 앱 형식 지오펜스 id 를 쓰지 않는 평가(단위 테스트·비 GPS)용. */
    public DayContext(LocalDate targetDate, ZoneId zone, Instant now, VerificationConfig config,
                      List<SyncSignal> signals, List<GeoAnchor> memberAnchors,
                      List<String> memberScreenApps, String memberId) {
        this(targetDate, zone, now, config, signals, memberAnchors, memberScreenApps, memberId, null, null);
    }
}

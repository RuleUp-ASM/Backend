...
public class ReportController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    중 필요한 쪽을 채운다. 사용자 신고에 `targetChallengeId` 를 함께 보내면 "어느 방에서 있었던
                    일인지"가 기록에 남는다.
                    ...
                    **같은 대상을 1주 안에 다시 신고하면 `duplicate:true`** 로 200 이 내려간다. 에러가 아니며
                    ...
                    `reason` 은 `SPAM` · `ABUSE` · `HATE` · `SEXUAL` · `VIOLENCE` · `OTHER` 중 하나다.
                    `OTHER` 를 고르면 `detail` 이 필수이고, 그 외에도 상세 내용을 적어 보내는 것을 권장한다
                    (최대 1000자). 본인은 신고할 수 없다.

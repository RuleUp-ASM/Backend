...
public class AdminController {
    ...
    @Operation(summary = "운영 공지 발행", description = """
            점검·장애·약관·종료 공지. **공지 탭 적재와 푸시 발송을 함께 한다**(2026-09-18).
            푸시는 마스터 토글·읽음 상태를 따르고, 21:00~08:00 KST에는 다음 08:00까지 보류한다.
            별도 그룹 토글은 없다. 응답은 비동기 팬아웃 접수 결과이며 푸시 전달 완료를 뜻하지 않는다.

            **`kind=MARKETING` 은 수신 대상을 제한한다.** 광고성 정보는 수신 **동의자에게만** 가고 푸시도 나간다

...
> **카테고리 코드는 한 벌이다.** 초안 시점에는 관심 카테고리(`WAKE_UP`)와 루틴 템플릿 카테고리(`WAKEUP`)가 갈라져 있어 정규화가 필요했지만, **12종 단일 코드**로 통일되면서 정규화 단계가 사라졌다(지금 스키마는 `V1__identity.sql`·`V2__routine_catalog.sql`). 지금은 문자열을 그대로 비교한다.
> 
...
flowchart TD
    ...
    SUM --> INT["관심 카테고리 집합<br/>(코드 한 벌 — 정규화 불필요)"]
...
- **관심사 코드 통일(V4 완료)**: 초안은 두 벌(`WAKE_UP` vs `WAKEUP`)을 정규화로 흡수했지만, 베이스라인 `V1__identity.sql`·`V2__routine_catalog.sql` 기준으로 **12종 단일 코드**로 통일해 정규화 단계를 없앴다. 근본 해결 쪽을 택한 것.

...
다만 **본질적으로 자동화가 불가능한 루틴**, 그리고 **자동 인증이 불가능한 예외 상황**을 위한 수동 인증은 남겨둔다. 실패 판정은 즉시 확정하지 않고 **이의 제기 창(3일)을 거쳐 확정**한다(솔로 제외).
...
- 실패 판정은 **잠정 실패 → 이의 제기 창(3일) → 확정**의 2단계로 처리한다(솔로 제외). 온도 가감 트리거는 확정 시점에만 발생한다.
...
| 수동 인증 무결성(가짜 사진 신고·관리자 VOID) | 본 스펙은 제출=SUCCESS(+방장/공동 관리자 승인 fallback)까지. MVP 이후 확장 가능성 |
...
SIG --> JUDGE["판정 모델"]
JUDGE --> RESULT["오늘자 SUCCESS / PENDING / 잠정 실패"]
```
...
D -->|No| E{"하루 마감?"}
E -->|아직| B
E -->|마감| F["잠정 실패 (INSUFFICIENT_DISTANCE/STEPS)<br/>확정은 이의 제기 창 후"]
```
...
W1 -->|마감| F1["잠정 실패 (INSUFFICIENT_USAGE)"]
end
...
B2 --> C2["사용 시간 계산"]
C2 --> MAX{"사용 > 목표?"}
MAX -->|"Yes (초과)"| F2["잠정 실패 (USAGE_EXCEEDED)<br/>확정은 이의 제기 창 후"]
MAX -->|No| W2{"창/하루 마감?"}
...
- **MAX(제약형)**: 사용 ≤ 목표면 충족, 초과 시 잠정 실패 (예: 쇼핑앱 30분↓, 폰 금지 창)
...
E -->|아직| B
E -->|"경과 (창 닫힘)"| W
W -->|"유효 신호 도착 (목표시각 이전)"| OK
W -->|"12시간 후·미충족"| F["잠정 실패<br/>→ 이의 제기 창 후 확정"]
...
- **확정 시점**: 창은 목표시각에 닫히지만 **판정 확정은 12시간 경과 후 배치에서** 한다 — 권한·기기 오류·Doze 배칭으로 “목표시각 이전 unlock” 신호가 몰아서 늦게 도착할 수 있기 때문. lag 안에 유효 신호가 오면 SUCCESS, 안 오면 잠정 실패 → 이의 제기 창을 거쳐 FAILED 확정. 그동안은 “실패 예정”으로 표시한다.
...
W -->|"마감·미충족"| F["잠정 실패<br/>(SLEPT_LATE / INSUFFICIENT_SLEEP)"]
F --> LOCK["이의 제기 창(3일) 종료 후<br/>FAILED 확정"]
```
...
| --- | --- | --- | --- | --- |
| **도달형 (ACHIEVEMENT)** | 목표를 달성해야 함 (출석·운동·기상·수면) | 충족 즉시 SUCCESS | 미충족 → 잠정 실패 | GEOFENCE(VISIT) · HEALTH · SCREEN_TIME(MIN) · WAKE · SLEEP |
| **제약형 (CONSTRAINT)** | 위반을 회피해야 함 (장소 회피·앱 절제) | 위반 즉시 잠정 실패 | 무위반 → SUCCESS | GEOFENCE(AVOID) · SCREEN_TIME(MAX) |
...
- **A. 확정 유예 — 늦게 오는 신호 (maxSignalLagHours)**
    - 판정(잠정 실패 전환)은 **창 닫힘 + lag 시점**에 한다. 배치가 `finalizeAfter < now`인 PENDING을 폴링해 판정한다.
    - lag 안에서는 PENDING 유지 → 늦게 온 유효 신호가 **PENDING → SUCCESS로 정정** 가능.
    - **lock(확정)된 FAILED → SUCCESS 역전은 금지.** 잠정 실패는 자동 신호로는 정정되지 않으며(lag가 이미 지났으므로), **이의 제기 승인(§8.7)으로만** SUCCESS로 전환된다.
    - lag 값: GPS·SCREEN_TIME·WAKE·HEALTH ≈ 12h(다음 새벽 판정), SLEEP ≈ 12h(익일 아침 일괄).
...
- **B. 라이프사이클 경계 처리 (중도 합류·탈퇴·삭제)**
    ...
    - **탈퇴**: 탈퇴일로 방 기록을 종결한다 — 탈퇴 시점 이후의 예정 인증일은 대상에서 제외(NOT_REQUIRED). 탈퇴일 당일의 미확정(PENDING) 건은 확정하지 않고 대상에서 제외한다(잔여 예정일 미계산 원칙). **이미 잠긴 확정 레코드(SUCCESS/FAILED)는 소급 삭제하지 않는다** — 탈퇴·삭제 패널티 판정(“본인/챌린지 내 success 기록 존재”)과 온도 계산의 원천이기 때문이다.
    - **챌린지 삭제(진행 중·참여자 0명)**: 삭제 시 **확정 레코드는 아카이브로 보존 후** 챌린지를 삭제한다. “챌린지 내 success 이력 존재” 판정 = 확정 SUCCESS 레코드 존재 여부(탈퇴한 과거 멤버의 것 포함). **미종결 이의 제기(잠정 실패의 3일 창 포함)가 있으면 삭제를 보류**한다(라이프사이클 스펙 §8-4).
- **C. 빈도형(FREQUENCY) 롤오버**
...
| 판정 모델 | polarity | 사용 Signal | 핵심 소비 필드 | 조기 판정 | 창 닫힘+lag | 실패 사유 | lag |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 위치 상태 변화 (VISIT) | 도달형 | `GEOFENCE` | `anchorId` `transition` `observedAt` `accuracy` `isMock` | 체류 충족 → SUCCESS | 미충족 → 잠정 실패 | `INSUFFICIENT_DWELL` | ≈12h |
| 위치 상태 변화 (AVOID) | 제약형 | `GEOFENCE` | `anchorId` `transition`(ENTER) `observedAt` | 위반 → 잠정 실패 | 무위반 → SUCCESS | `ENTERED_FORBIDDEN_ZONE` | ≈12h |
| 이동 거리 | 도달형 | `HEALTH` | `metric` `readings[]` `recordingMethod` `originPackage` `recordId` | 누적 ≥ 목표 → SUCCESS | 미달 → 잠정 실패 | `INSUFFICIENT_DISTANCE/STEPS` `UNTRUSTED_HEALTH_SOURCE` | ≈12h |
| 스크린 타임 (MIN) | 도달형 | `SCREEN_TIME.usageEvents` | `RESUMED/PAUSED/STOPPED` + ts | 사용 ≥ 목표 → SUCCESS | 미달 → 잠정 실패 | `INSUFFICIENT_USAGE` | ≈12h |
| 스크린 타임 (MAX) | 제약형 | `SCREEN_TIME.usageEvents` | 동일 + `windowAnchor` | 초과 → 잠정 실패 | 무위반 → SUCCESS | `USAGE_EXCEEDED` | ≈12h |
| 기상 | 도달형 | `SCREEN_TIME.screenEvents` (+`HEALTH` 보조 검토) | `firstUnlock` `firstScreenOn` `deviceSecure` (+걸음 첫 `startTime`) | 목표시각 前 신호 → SUCCESS | lag 후 미신호 → 잠정 실패 | `WOKE_UP_LATE` | ≈12h |
| 수면 ⚠️ | 도달형 | `SLEEP.sessions` | `start` `stages[]` `durationMillis` + origin 메타 | — (익일 일괄) | 미충족 → 잠정 실패 | `SLEPT_LATE` `INSUFFICIENT_SLEEP` | ≈12h(익일 아침) |

> 잠정 실패의 확정(FAILED lock)은 §8.7의 이의 제기 창(3일) 종료 후. 솔로는 잠정 단계 없이 창 닫힘+lag 시 즉시 확정(§8.7).
> 
...
- **A. VISIT — 도달형**
    ...
    - **판정**: 체류 충족 즉시 SUCCESS(lock). 창 닫힘+lag까지 미충족이면 잠정 실패(`INSUFFICIENT_DWELL`) → §8.7.
- **B. AVOID — 제약형**
    ...
    - **판정 알고리즘**: 평가창 내 유효한 ENTER 발생 → 위반 → **잠정 실패**. 창 닫힘까지 무위반이면 SUCCESS.
    ...
    - **판정**: 위반 시 잠정 실패(`ENTERED_FORBIDDEN_ZONE`) → §8.7. 창 닫힘+lag 무위반 → SUCCESS.
- 8.3.2 이동 거리 판정 모델 (HEALTH)
    ...
    - **판정**: 누적 충족 즉시 SUCCESS(lock). 창 닫힘+lag 미달 → 잠정 실패(`INSUFFICIENT_DISTANCE` / `INSUFFICIENT_STEPS`) → §8.7.
- 8.3.3 스크린 타임 판정 모델 (SCREEN_TIME)
    ...
    - **판정 알고리즘**: RESUMED→PAUSED(STOPPED 보정) 페어링으로 세션 길이를 구해 평가창 내 합산.
        - **MIN (도달형)**: 합산 ≥ `targetMinutes` → 충족 즉시 SUCCESS. 창 닫힘 미달 → 잠정 실패(`INSUFFICIENT_USAGE`).
        - **MAX (제약형)**: 합산 > `targetMinutes` → **잠정 실패**(`USAGE_EXCEEDED`). 창 닫힘 무위반 → SUCCESS.
...
- 8.3.4 기상 판정 모델 (WAKE)
    ...
    - **판정 알고리즘**: 당일 첫 unlock(폴백: 첫 screen-on) 시각 ≤ `targetWakeTime` → SUCCESS. 목표시각 경과 후에도 서버 상태는 PENDING 유지(UI만 잠정 표시), **lag(≈12h) 경과 후 배치에서** 유효 신호 없으면 잠정 실패(`WOKE_UP_LATE`) → §8.7.
...
- 8.3.5 수면 판정 모델 (SLEEP)
    ...
    - **판정 알고리즘**:
        ...
        - `취침 ≤ targetBedtime` / `수면 ≥ targetSleepMinutes` 충족 여부 판정. 미충족 → 잠정 실패(`SLEPT_LATE` / `INSUFFICIENT_SLEEP`) → §8.7.
    - **이상값 필터**: HEALTH와 동일한 오리진 게이트(MANUAL 거부·화이트리스트)·`record.id` dedup 공유.
    - **판정**: 신호가 익일 아침에 오므로 조기 판정 없음. lag ≈ 12h — 다음 새벽 배치에서 판정.
...
- 8.4 평가 구조 — 스테이트리스 우선 (상태 테이블 최소화)
    ...
    **저장하는 것 (write는 4종뿐)**
    ...
    2. **잠정 실패 레코드** — 창 닫힘+lag 배치가 쓴다. 이의 제기 창(3일)의 시작점이자 이의 제기 대상 식별자. 이의 제기 승인 시 SUCCESS 확정 레코드로 전환, 미제기/기각 시 FAILED 확정 레코드로 잠금.
    3. **확정 레코드 (불변)** — lock이 걸리는 순간에만 쓴다:
        - 도달형 조기 SUCCESS (역전 불가이므로 즉시 lock 가능)
        - 이의 제기 창 종료 후 배치가 잠그는 최종 FAILED / 이의 제기 승인에 의한 SUCCESS
        - 솔로 챌린지의 창 닫힘+lag 시점 FAILED (잠정 단계 생략 — §8.7)
    4. **수동 인증 제출·승인 레코드** (§10) 및 **이의 제기 제출·처리 레코드** (§8.7).
    ...
    **조회 시 평가 (progress API·verification API 공통)**
    ...
    status(member, date) =
      확정 레코드 있음        → 그 값 (불변)
      잠정 실패 레코드 있음    → FAILED_PROVISIONAL (이의 제기 가능 여부·마감 포함)
      없음 & date가 대상일    → 원본 신호 + config로 §8.3 알고리즘 실행
                               → 충족이면 SUCCESS(+ 확정 레코드 write)
                               → 아니면 PENDING (아무것도 쓰지 않음)
      없음 & 비대상일         → NOT_REQUIRED
    ```
    ...
    **이 구조의 효과**
    ...
    - **lock된 FAILED→SUCCESS 역전 금지가 구조적으로 보장** — 확정 FAILED는 불변 레코드로만 존재한다. 실패의 유일한 구제 경로는 잠정 실패 단계의 이의 제기(§8.7)뿐이다.
    - **온도 가감 트리거는 확정 시점에만 발생** — 잠정 실패는 온도에 반영하지 않는다. 이의 제기로 뒤집히면 온도 복원 문제 자체가 생기지 않는다(실패가 확정된 적이 없으므로).
    - 새벽 배치의 역할: ① `finalizeAfter < now`인 미판정 대상일 → 잠정 실패(솔로는 FAILED 확정), ② 이의 제기 창이 종료된 잠정 실패 → FAILED 확정. 두 가지로 단순화.
...
- 8.5 신호/권한 부재일 처리
    - 창 닫힘 + lag까지 필수 신호 미수신 → 잠정 실패(솔로는 FAILED 확정)로 처리하되, **gaps[] 사유로 구분 기록**:
    ...
    - 일시적 불가면 창 마감(판정 확정) 전에 수동 인증 제출 가능(§10.2) — 그룹·솔로 공통.
...
- 8.6 다중 인증방식 결합 (methodCombine)
    ...
    - 확정 타이밍은 결합 결과에서 **가장 늦은 창/lag**를 따른다(AND면 모든 모델의 창이 닫혀야 판정).

### 8.7 실패 확정과 이의 제기 (3일 창)

실패는 즉시 확정하지 않는다. **잠정 실패 → 이의 제기 창(3일) → 확정**의 2단계로 처리한다.

```
PENDING ──(창 닫힘 + lag, 미충족/위반)──> 잠정 실패 (FAILED_PROVISIONAL)
잠정 실패 ──(3일 내 이의 제기 없음 / 기각)──> FAILED 확정 (lock, 온도 반영)
잠정 실패 ──(3일 내 이의 제기 → 승인)──> SUCCESS 확정 (verifiedVia=OBJECTION)
```

- **이의 제기 창**: 잠정 실패 전환 시점부터 **3일**. 창이 끝나면 새벽 배치가 FAILED로 잠근다(온도 가감 트리거는 이때 발생).
- **제출**: 해당 멤버 본인. 형식은 **사진을 포함한 글 혹은 글만** (라이프사이클 스펙 §7-3과 통일). 잠정 실패 상태의 일자에만 제출 가능, 일자당 1회.
- **처리 주체**: **방장(OWNER) / 공동 관리자(MANAGER)** — 라이프사이클 §7-1 권한 매트릭스의 “이의 제기 처리”와 동일. 방 내부 처리이며 서비스 운영자가 아니다.
- **승인 결과**: 해당 일자를 **SUCCESS로 확정**(`verifiedVia=OBJECTION`)하고 진행률에 반영한다. 잠정 실패는 온도에 반영된 적이 없으므로 **온도 복원 문제가 발생하지 않는다.**
- **기각 결과**: 즉시 FAILED 확정(lock). 재제기 불가.
- **솔로(SOLO) 예외**: 처리 주체가 본인뿐이라 이의 제기가 무의미하다 → **솔로는 잠정 실패 단계 없이 창 닫힘+lag 시점에 즉시 FAILED 확정**한다. 대신 창 마감 전에는 수동 인증(§10.2)으로 스스로 구제할 수 있다.
- 이의 제기 `type`은 MVP에서 `FAILURE` 하나다. `ABUSE_PENALTY`는 Phase 2 어뷰징 처벌 도입 시를 위한 예약값(§9.3).
...
- 대신 **서버가 판정 시점에 과한 이상값을 감지하면, 주변 정상 데이터를 보고 예상값으로 대체(보간)해 판정**한다:
    ...
    - **보정 불가 시**: 양옆에 정상 점이 없어 보간할 수 없으면 해당 구간의 신호만 폐기한다. 그로 인해 신호가 부족해 미충족이 되면 일반적인 **잠정 실패 → 이의 제기(§8.7)** 경로를 탄다 — 즉 유저 구제는 별도 처벌·어필 시스템이 아니라 기존 이의 제기 채널 하나로 일원화된다.
...
## 10. 수동 인증 & 최종 fallback (방장/공동 관리자 승인)

> 자동 인증이 끝내 안 되는 경우의 구제는 “일단 통과”가 아니라 **방장(OWNER) 또는 공동 관리자(MANAGER)가 승인(approval)** 해야 하는 이의제기 성격의 최종 fallback(수동 인증)이다. (승인 권한은 라이프사이클 스펙 §7-1의 “이의 제기 처리”와 동일)
> 
...
자동 챌린지인데 권한 먹통·기기 미지원·신호 부재로 무조건 실패는 억울하다. 단 아무 때나 수동으로 도망가면 자동 인증이 무의미해지므로 **트리거 + 승인**을 건다.

- **트리거**
    - 구조적 불가(권한 거부·기기 미지원) → 처음부터 fallback 제공.
    - 일시적 불가(평소 자동인데 오늘 신호 없음) → “오늘은 수동으로 증명하기” 노출.
- - **제출 가능 시점**: **해당 일자의 판정이 확정되기 전까지** (솔로: 창 닫힘+lag 시점 / 그룹: 이의 제기 창 마감 시점) — 자동 인증이 안 될 때의 대용이므로 창 진행 중 선제출도 허용된다. 그룹은 잠정 실패 전환 후에도 이의 제기 창 안에서는 제출 가능하며, 승인되면 잠정 실패를 대체해 SUCCESS로 확정된다.
- **핵심 규칙**
    - 제출은 **잠정 상태일 뿐, 자동 통과가 아니다.** (그룹 기준)
    - **방장 또는 공동 관리자가 approval 해야 비로소 인증 확정**된다(거절 가능). 완전히 이의제기/승인 플로우.
    - 제출 형식은 이의 제기와 통일: **사진을 포함한 글 혹은 글만.**
    - 남용 방지를 위해 한도(1달에 챌린지 방 1개당 3회)를 둔다. **한도 초과 = 제출 거부일 뿐, 그날이 실패로 확정되는 것이 아니다** — 해당 일자는 기존 자동 판정 경로를 그대로 따른다.
    - **솔로(SOLO)**: 승인 주체가 본인뿐이라 자기 승인이 무의미하다 → 정규 수동과 동일하게 **제출 즉시 SUCCESS**(`verifiedVia=MANUAL_FALLBACK`), 한도는 동일 적용. 솔로는 확정 후 이의 제기가 없으므로(§8.7) 이 사전 구제가 유일한 예외 경로다.

```mermaid
flowchart TD
T["신호 부재"] --> BTN["유저: '수동으로 증명' 제출"]
BTN --> WIN{"제출 기한 내?<br>솔로: 인증 창 마감 전<br/>그룹: 이의 제기 창 마감 전"}
WIN --> LIM{"한도 내?"}
WIN -->|기한 경과| CLOSED["제출 불가"]
LIM -->|초과| DENY["제출 거부 안내<br/>(자동 판정 경로 유지 — 실패 확정 아님)"]
LIM -->|OK| SOLO{"솔로 챌린지?"}
SOLO -->|Yes| OKS["즉시 SUCCESS 확정"]
SOLO -->|No| PEND["잠정 PENDING_APPROVAL<br/>(통과 아님 · 잠정 실패 후에도 제출 가능)"]
PEND --> OWNER{"방장/공동 관리자 승인?"}
OWNER -->|approve| OK["SUCCESS 확정<br/>(잠정 실패였다면 대체)"]
OWNER -->|reject| FAIL2["해당 제출 기각<br/>→ 기존 판정 경로 유지"]

classDef wait fill:#FFF3E0,stroke:#EF6C00,color:#E65100;
class PEND wait;
```

> 폴백 제출이 기각돼도 그날이 곧바로 실패 확정되는 것은 아니다 — 자동 신호가 창 안에 들어오면 SUCCESS가 될 수 있고, 끝내 미충족이면 §8.7의 잠정 실패 경로를 탄다(같은 일자에 대한 이의 제기는 폴백 기각 사유와 중복 심사를 피하기 위해 동일 근거로는 불가).
> 
...
- **실패 즉시 확정 (폐기)**: 창 닫힘+lag 시점에 FAILED를 즉시 lock하는 설계였으나, 이의 제기(3일)와 구조적으로 충돌해 **잠정 실패 → 이의 제기 창 → 확정**의 2단계로 변경. 온도 반영을 확정 시점으로 미뤄 “이의 제기 승인 시 온도 복원” 문제를 원천 제거했다.
...
- 🔶 이의 제기 남용 한도 필요 여부 (fallback 한도는 월 3회/챌린지로 확정)

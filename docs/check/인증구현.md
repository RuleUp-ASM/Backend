...
F --> LOCK["D+2 00:00 KST 확정 · 이의 마감<br/>구제는 확정 전 유예에 신청"]
```
...
S1 : SUCCESS (lock)
L1 : FAILED (lock)
P1 --> S1 : 유예 중 이의 자동 인용 (verifiedVia=APPEAL)
}
...
- **A. 확정 유예 — 늦게 오는 신호 (유형 공통, 하루)**
    ...
    - **확정된 FAILED → SUCCESS 역전은 자동 신호로는 불가능**하다. 정상 이의는 확정 전 유예에 신청한다(§8.7). 레거시 정정 경로도 이의 마감 검사를 우회하지 않는다.
...
- **A. VISIT — 도달형**
    ...
    - **판정**: 체류 충족 즉시 SUCCESS(lock). D+2 00:00 KST 까지 미충족이면 FAILED(`INSUFFICIENT_DWELL`) 확정. 구제는 확정 전 유예 구간의 §8.7 이의로 신청한다.
- **B. AVOID — 제약형**
...
PENDING ──(D+1 유예 중 이의 자동 인용)───────────────> SUCCESS (verifiedVia=APPEAL, 비동기 점수 반영)
```
...
2. **이대로면 실패인가** — 정상 신청 구간은 유예 중 FAIL_EXPECTED(DB: PENDING)다. 이미 완료된 건과 아직 채울 기회가 남은 건은 거절한다. 레거시 FAILED도 기한을 별도로 검사하며, 정상 D+2 확정 건은 이미 마감됐다.
...
- **인용 결과**: 실패 예정을 완료로 정정하고(`verifiedVia=APPEAL`) 진행률·통계·연속 기록을 다시 계산한다. 점수 반영 의사는 같은 트랜잭션의 아웃박스에 기록하고, 커밋 이후 정상 성공과 같은 기준으로 사이클·현재 점수를 재계산한다. `restored.scoreDelta=0`은 접수 시 동기 지급분이며 최종 증분이 아니다. [APL-04 검증 절차](APL-04-appeal-verification.md)를 따른다.
...
- 대신 **서버가 판정 시점에 과한 이상값을 감지하면, 주변 정상 데이터를 보고 예상값으로 대체(보간)해 판정**한다:
    ...
    - **보정 불가 시**: 양옆에 정상 점이 없어 보간할 수 없으면 해당 구간의 신호만 폐기한다. 그로 인해 신호가 부족해 미충족이 되면 일반적인 **실패 예정 → 유예 중 이의 자동 인용(§8.7)** 경로를 탄다 — 즉 유저 구제는 별도 처벌·어필 시스템이 아니라 이의 채널 하나로 일원화된다.

...
- **스코어**: `challenge_stats.recent_joins_24h` = 최근 24시간 안에 발생한 이 방의 신규 참여 수
    ...
    - **지수 감쇠·방 나이 보정은 쓰지 않는다.** 24시간 창 자체가 감쇠 역할을 하고, 가중치를 넣는 순간 값이 배치 시각에 의존해 재계산 원천(MySQL 컬럼)과 투영 순서가 어긋난다.
...
- **갱신 주기 = 5분 / 계산 방식**
    - `PopularityRefreshJob`(cron `0 */5 * * * *`, KST)이 `challenge_join_events.joined_at` 기준으로 최근 24시간 가입을 집계해 `challenge_stats` 를 500행 청크로 갱신한다.
    - 순위 원천은 **Redis ZSET**(`trending:*`)이고, `ExploreIndexJobs` 가 같은 5분 주기로 투영한다. Redis 장애·워밍업 미완료 시 503을 반환하고, 복구 후 스윕으로 재투영한다.
    - 집계·투영 배치는 MySQL **ShedLock**으로 다중 인스턴스 중복 실행을 제어한다.
    - Redis 제공 비율은 `trending_redis_serve_ratio`, 회로 상태는 `explore_redis_circuit_open`, 집계 지연은 `popularity_last_success_age`로 본다.
...
- 정상 경로는 10분마다 집계한 Redis 카테고리 개수 HASH를 읽으며, Redis 장애 시 `challenges` 원천을 집계해 폴백한다.
  카운터 컬럼은 증감 지점(생성·가입·탈퇴·강퇴·종료·삭제)마다 드리프트가 생기고 보정 배치가 또 필요한데,
  방 수는 만 단위라 매 갱신마다 다시 세는 편이 단순하고 정확하다.
- **갱신 시점**: 정상 조회는 10분 정기 집계 결과를 사용한다. 방 생성·종료·삭제 직후의 즉시 반영은 보장하지 않는다.
- 기존 `ChallengeGridChanged`의 Caffeine evict는 Redis HASH를 갱신하지 않으므로, 이를 정상 조회의 갱신 시점이나 TTL 보장으로 해석하지 않는다.

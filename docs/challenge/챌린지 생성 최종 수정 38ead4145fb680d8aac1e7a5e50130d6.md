...
- 제목/설명(선택) → **LLM 루틴 템플릿 매칭 추천(2초 이내)** → 폼 갱신. 매칭/LLM 실패도 200 + **체크형(SELF_CHECK) 수동 폴백**(자동 인증 불가, AUTO 선택 불가).
...
- **4.3 인증 모델 = 루틴 템플릿** — `verificationMethods` 배열 폐기. `templateId`(**자동 인증 가능 루틴만** 담은 카탈로그 매칭) + `selectedMethod(AUTO/MANUAL)` + `verification` 스냅샷(`verificationType`·`signalSource`·`wearableReq`·`requiredPermissions`·`externalService`) + `params`(목표값) 모델. **매칭되면 자동 인증 가능**, **매칭 실패 = 자동 인증 불가 → 체크형(SELF_CHECK) 수동만, AUTO 선택 불가**(“루틴을 못 찾음”이 아니라 “자동 인증 불가” 메시지).
...
- **4.5 모더레이션 = 이미지 기준 가시성 게이트** — 생성은 항상 성공. `moderationStatus`(PENDING_REVIEW→APPROVED/REJECTED) 축. **가시성 게이트는 대표 이미지 기준**: 이미지 **없으면 생성 즉시 APPROVED(공개)**, 이미지 **있으면 PENDING_REVIEW** → 생성/이미지변경 후 **비동기 SafeSearch** 검수 통과 시 공개. 위반 시 REJECTED + 알림 + **1시간 수정창**(이미지 교체), 미수정 시 영구 닫힘. **자동 판정만**. *AI 미가용(UNAVAILABLE)이면 PENDING_REVIEW 유지 + 재검 배치가 복구 즉시 공개(§8), 결론 후 재검 없음.* — **텍스트(이름)는 LLM 모더레이션을 두지 않는다**: 생성 시 규칙기반 **blocklist(동기)**로 명백 비속어 차단 + 제목/설명 기반 추천 시 **draft STEP1 게이트**(무의미·불순 내용 usable=false)로 거른다(LLM 호출 1회로 매칭+검수 통합). 업로드(`POST /challenges/image`) 동기 SafeSearch도 유지(사전 차단).
...
flowchart TD
    ...
    AI -->|예| REC["LLM 루틴 템플릿 매칭<br/>(매칭=자동인증 가능 · 실패=자동인증 불가→체크형 수동)"]
    REC --> EDIT
    EDIT --> CREATE["챌린지 생성"]
    ...
    subgraph MOD ["자동 모더레이션"]
        direction TB
        ASYNC["이미지 있으면: 비동기 SafeSearch 검수<br/>이미지 없으면: 즉시 APPROVED (검수 생략)"]
        ASYNC --> GATE{"통과?"}
...
- **모더레이션**: 가시성 게이트는 **이미지 기준**(없으면 즉시 공개, 있으면 비동기 SafeSearch 후 공개) + 업로드 시 동기 SafeSearch. **이름 LLM 모더레이션은 제거** — 대신 규칙기반 **blocklist(동기)** + 추천 **draft STEP1 게이트**로 텍스트를 거른다(LLM 호출 1회로 통합). 검색 노출은 APPROVED만. AI 미가용 시 이미지 검수는 재검 배치로 복구(§4.5) — AI 결론 후 재검 없음.

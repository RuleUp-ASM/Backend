...
- **4.2 진입은 LLM 비강제** — 카테고리 추천 버튼은 LLM을 호출하지 않고 클라가 기본 템플릿으로 폼을 즉시 채운다. LLM 추천(`/recommendation`)은 제목 기반 루틴 템플릿 매칭으로 폼을 채우는 **선택 단계**.
...
- **4.4 권한** — AUTO 선택 시 `grantedPermissions`를 요청에서 검증(`ROUTINE_PERMISSION_REQUIRED`), 응답 `verification.requiredPermissions` 제공. *주의: 인증 스펙은 무거운 권한(백그라운드 위치·사용정보 접근)을 최초 진입 셋업으로 이연하므로, 무거운 권한까지 생성 시 강제할지는 인증 셋업과 정렬 필요 — 현 API 계약 기준 유지.*
- **4.5 모더레이션 = 비동기 가시성 게이트** — 생성은 항상 성공. `moderationStatus`(PENDING_REVIEW→APPROVED/REJECTED) 축 추가. 검수 전 타인 비노출, 통과 후 공개·가입. 위반 시 REJECTED + 알림 + **1시간 수정창**(솔로·그룹 동일), 미수정 시 영구 닫힘. **자동 판정만**(어드민 리뷰 없음). REJECTED 시 진행 중 멤버 기록/패널티 별도 처리 없음(비노출 게이트 + 1h 창으로 영향 작음).
...
- **4.7 상태 축 분리** — lifecycle `status`(RECRUITING/ACTIVE/COMPLETED) · 모더레이션 `moderation_status` · 멤버 `setup_status`는 **서로 다른 축**. 시작 전(ACTIVE 이전)에만 PATCH.
...
flowchart TD
    ...
    START --> RECO["카테고리 선택 / 추천 버튼"]
    RECO --> FILL["기본 템플릿으로 폼 즉시 채움<br/>(LLM 호출 없음)"]
    ...
    subgraph MOD ["자동 모더레이션"]
        direction TB
        ASYNC["비동기 자동 모더레이션<br/>(이름 LLM · 이미지 SafeSearch)"]
        ASYNC --> GATE{"통과?"}
...
- **모더레이션**: 자동(이름 LLM·이미지 SafeSearch) 비동기. 명백 비속어는 규칙 기반 blocklist로 동기 사전 차단 가능. 검색 노출은 APPROVED만.
- **권한 모델 정렬**: AUTO `grantedPermissions` 검증이 인증 스펙의 "무거운 권한 셋업 이연"과 겹치는 영역 — 추후 정렬.
...
- `GET /challenges`(탐색/검색·랭킹)·`GET /recommendations/routines` → 탐색·추천 스펙(랭킹 + APPROVED 노출 게이트).
- "내 챌린지 목록"은 `GET /verifications/progress`가 커버. 필요 시 가벼운 `GET /challenges/mine`만.
